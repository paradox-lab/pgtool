...
def make_pat_go():
    from pgtool.ProgramTool.keywords import goKwlist,goBuiltins
    kw = r"\b" + any("KEYWORD", goKwlist) + r"\b"
    builtin = r"([^.'\"\\#]\b|^)" + any("BUILTIN", goBuiltins) + r"\b"
    comment = any("COMMENT", [r"//[^\n]*"])
    stringprefix = r"(?i:r|u|f|fr|rf|b|br|rb)?"
    sqstring = stringprefix + r"'[^'\\\n]*(\\.[^'\\\n]*)*'?"
    dqstring = stringprefix + r'"[^"\\\n]*(\\.[^"\\\n]*)*"?'
    sq3string = stringprefix + r"'''[^'\\]*((\\.|'(?!''))[^'\\]*)*(''')?"
    dq3string = stringprefix + r'"""[^"\\]*((\\.|"(?!""))[^"\\]*)*(""")?'
    gocsstring=stringprefix+r'/\*[^"\\]*(\\.[^"\\\n]*)*"?'
    gocstring=stringprefix + r'/\*[^"\\]*((\\.|"(?!""))[^"\\]*)*\*/?'
    string = any("STRING", [sq3string, dq3string, sqstring, dqstring,gocstring,gocsstring])
    return kw + "|" + builtin + "|" + comment + "|" + string +\
           "|" + any("SYNC", [r"\n"])

def make_pat_html():
    stringprefix = r"(?i:r|u|f|fr|rf|b|br|rb)?"
    builtin = stringprefix + r"<[^'\\]*((\\.|'(?!''))[^'\\]*)*(>)?"
    return  builtin

def prog(lang):
    if lang=="go":
        return re.compile(make_pat_go(), re.S)
    else:
        return re.compile(make_pat(), re.S)
